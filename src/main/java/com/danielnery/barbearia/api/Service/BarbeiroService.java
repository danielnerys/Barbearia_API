...
public class BarbeiroService {
    ...
    public BarbeiroResponse cadastrar(BarbeiroRequest request) {
        ...
        novoBarbeiro.setNome(request.nome().trim());
        novoBarbeiro.setEspecialidade(request.especialidade());
        novoBarbeiro.setAtivo(request.ativo() !=null ? request.ativo() : true);

        return toResponse(barbeiroRepository.save(novoBarbeiro));
    }
